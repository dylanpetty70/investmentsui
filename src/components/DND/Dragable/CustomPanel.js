			...
			<div style={{width: '22vw', border: '1px solid', backgroundColor: 'lightGrey', position: 'absolute', right: '5px', top: '128px', height: '75vw'}}>
				<Card style={{marginTop: '5px', marginBottom: '5px', marginLeft: '2px', marginRight: '2px'}}>
					...
					<Card.Body>
						<Form inline='true' style={{marginLeft: '10px'}}>
							...
							<Button variant="outline-primary" style={{marginLeft: '30px'}} onClick={() => {(!isNaN(Number(this.state.tempScale))) ? this.props.handleChangeScale(this.state.tempScale, this.props.envOptions.current) : this.setState({...this.state, scaleError: true})}}>Change Scale</Button>
						</Form>
					</Card.Body>
				</Card>
				...
				<Card style={{marginTop: '5px', marginBottom: '5px', marginLeft: '2px', marginRight: '2px'}}>
					...
					<Card.Title style={{fontSize: '16px'}}>Creatures/Humanoids</Card.Title>
						...
						<Form.Group>
						<Form.Control placeholder={this.state.tempTokenScale.creature} style={{width: '50px'}} custom onChange={(text) => {this.setState({...this.state, tempTokenScale: {...this.state.tempTokenScale, creature: text.target.value}})}}/>
						</Form.Group>
							</Col>
							</Row>
						</Container>
						...
						<Typeahead
							...
							<Col style={{marginLeft: '150px'}}>
						<Form.Group>
						<Form.Control placeholder={this.state.tempTokenScale.background} style={{width: '50px'}} custom onChange={(text) => {this.setState({...this.state, tempTokenScale: {...this.state.tempTokenScale, background: text.target.value}})}}/>
						</Form.Group>
							</Col>
							</Row>
						</Container>
						...
						<Typeahead
							...
							<Col style={{marginLeft: '150px'}}>
						<Form.Group>
						<Form.Control placeholder={this.state.tempTokenScale.scene} style={{float: 'left', width: '50px'}} custom onChange={(text) => {this.setState({...this.state, tempTokenScale: {...this.state.tempTokenScale, scene: text.target.value}})}}/>
						</Form.Group>
							</Col>
							</Row>
						</Container>
